...
ap.add_argument ("gff_pc", type=str ) #repeats 
...
for line1 in (open(fn1)):
    ...
    if field[2] == "similarity":
        #pc_information.append((field[0],int(field[3]) - 1000 ,int (field[4]) + 1000 ,field[6]))
        pc_information.append((field[0],int(field[3]) ,int (field[4]) ,field[6]))
  ...
  #print (line)
    ...
    if (my_gff["type"] == 'transcript'):
        for pc in pc_information:
            if (my_gff["scaffold"] == pc[0]):
                if (int (my_gff["start"]) >=  int(pc[1]) and int (my_gff["end"]) <= int (pc[2]) and my_gff["strand"] == pc[3]) or (int (my_gff["start"]) <=  int(pc[1]) and int (my_gff["end"]) >= int (pc[1]) and my_gff["strand"] == pc[3]) or (int (my_gff["start"]) <=  int(pc[2]) and int (my_gff["end"]) >= int (pc[2]) and my_gff["strand"] == pc[3]):
